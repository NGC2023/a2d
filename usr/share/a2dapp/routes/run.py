...
def generate_cron_job(interval_minutes, interval_hours, interval_hours_uni):
    ...
    if interval_minutes == 0 and interval_hours == 0:
        cron_jobs.append(f"*/15 * * * * /usr/bin/python /usr/bin/a2d/runscripts.py")
    elif interval_minutes == 1 and interval_hours == 0:
        cron_jobs.append(f"*/2 * * * * /usr/bin/python /usr/bin/a2d/runscripts.py")
    elif interval_minutes != 0 and interval_hours == 0:
        cron_jobs.append(f"*/{interval_minutes} * * * * /usr/bin/python /usr/bin/a2d/runscripts.py")
    elif interval_minutes == 0 and interval_hours != 0:
        cron_jobs.append(f"0 */{interval_hours} * * * /usr/bin/python /usr/bin/a2d/runscripts.py")    
    elif interval_minutes != 0 and interval_minutes != 1 and interval_hours != 0 and interval_hours != 1:
        cron_jobs.append(f"*/{interval_minutes} */{interval_hours} * * * /usr/bin/python /usr/bin/a2d/runscripts.py")
    else:
        cron_jobs.append(f"0 */{interval_hours_uni} * * * /usr/bin/python /usr/bin/a2d/runscripts.py")
        cron_jobs.append(f"0-59/{interval_minutes} * * * * /usr/bin/python /usr/bin/a2d/runscripts.py")
    return cron_jobs
...
def add_cronjob():
    ...
    command = '/usr/bin/python /usr/bin/a2d/runscripts.py'
...
def start_service():
    if os.path.isfile('/etc/a2d/.creds/a2d_AnDinfo.conf'):
        ...
        if runstatus == "Stopped":
            ...
            add_cronjob()
            run_command = ["nohup", "/usr/bin/a2d/scripts/run_a2d.sh", "&"]
